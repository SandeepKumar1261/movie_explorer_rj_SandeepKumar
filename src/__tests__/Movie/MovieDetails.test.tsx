...
// Mock the toggleWishList API function
jest.mock('../../Utils/Api', () => ({
...
const mockMovie = {
  ...
  duration: 139,
  inWatchlist: false, // Fixed the typo here
};
...
describe('MovieDetails Component', () => {
  ...
  test('renders movie details correctly', () => {
    ...
    expect(screen.getByRole('img')).toHaveAttribute('src', mockMovie.banner_url);
  });
  ...
  test('displays loading state when toggling watchlist', async () => {
    (toggleWishList as jest.Mock).mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 100)));
    render(<MovieDetails movie={mockMovie} />);
  ...
  test('handles error when toggling watchlist fails', async () => {
    ...
    await waitFor(() => {
      expect(toggleWishList).toHaveBeenCalledWith(mockMovie.id, 'mock-token');
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error toggling wishlist:', expect.any(Error));
